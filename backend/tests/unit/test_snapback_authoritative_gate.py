...
def test_authoritative_gate_promotion_when_all_gates_cleared():
    ...
    assert verdict.expectancy_without_top_1pct_profits > 0.0
    assert verdict.checks["positive_under_3x_costs"] is True
    assert verdict.checks["positive_after_top_1pct_profit_removal"] is True
...
def test_tail_removal_blocks_edge_carried_only_by_three_outliers():
    ...
    assert verdict.expectancy_without_top_1pct_profits < 0
    assert verdict.checks["positive_after_top_1pct_profit_removal"] is False
    assert verdict.promoted is False
