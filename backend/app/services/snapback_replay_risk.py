"""Research-only account risk simulator.

This is the in-memory reservation model the replay harness uses to simulate capital
constraints over historical tapes. It is deliberately NOT the live execution
authority: reservations here live in one process and are forgotten on restart, which
is fine for a replay and unacceptable for real money. Live risk belongs to the
canonical RiskEngine via CanonicalExecutionService.
"""
