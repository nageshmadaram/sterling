...
def evaluate_authoritative_snapback_gate(
    ...
    if n_trades == 0:
        return AuthoritativeGateVerdict(
            promoted=False,
            total_sessions=entry_sessions_count,
            completed_trades=0,
            net_expectancy=0.0,
            lower_95_ci=0.0,
    ...
    costs = np.array(statutory_costs, dtype=float) if len(statutory_costs) == n_trades else np.zeros(n_trades)

    net_expectancy = float(np.mean(pnls))
    
    # 2x and 3x cost stress
    pnls_2x = pnls - costs  # Deduct an extra 1x costs for 2x cost total
    pnls_3x = pnls - (2.0 * costs)
    
    expectancy_2x = float(np.mean(pnls_2x))
    expectancy_3x = float(np.mean(pnls_3x))

    # Day-clustered / block bootstrap lower 95% CI
    if n_trades >= 10:
        boot = [np.mean(np.random.choice(pnls, size=n_trades, replace=True)) for _ in range(1000)]
        lower_95_ci = float(np.percentile(boot, 2.5))
    else:
    ...
    dd = peak - cum
    max_dd_val = float(np.max(dd)) if len(dd) > 0 else 0.0
    initial_cap = max(1.0, float(np.max(peak)) if np.max(peak) > 0 else 100000.0)
    max_dd_pct = (max_dd_val / initial_cap) * 100.0
    ...
    checks["independent_sessions_ge_60"] = entry_sessions_count >= 60
    if not checks["independent_sessions_ge_60"]:
        reasons.append(f"Independent sessions {entry_sessions_count} < required 60")
    ...
    if not checks["positive_lower_95_ci"]:
        reasons.append(f"Lower 95% CI ({lower_95_ci:.2f}) is not strictly positive (>0)")
    ...
    # Gate 6: Drawdown within loss budget
    checks["drawdown_within_budget"] = max_dd_pct <= max_allowed_drawdown_pct
    if not checks["drawdown_within_budget"]:
        reasons.append(f"Max MTM drawdown {max_dd_pct:.2f}% exceeds budget {max_allowed_drawdown_pct:.2f}%")
